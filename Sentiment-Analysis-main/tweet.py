...
dataset_trian = dataset['train']
dataset_trian.set_format("pandas")

parser = argparse.ArgumentParser()
parser.add_argument('--model', default='bert-base-cased', type=str, help='bert model')
...
data = pd.concat([dataset_trian[:-1]], axis=0).reset_index(drop=True)
MAX_LEN = max([len(sent) for sent in data.text.values])
print(MAX_LEN)
# print(data.size)
...
# Encode 我们连接的数据
encoded_comment = [tokenizer.encode(sent, add_special_tokens=True, truncation=True, max_length=512) for sent in data.text.values]
...
class BertClassifier(nn.Module):
    def __init__(self, ):
        ...
        self.classifier = nn.Sequential(
            nn.LayerNorm(D_in),
            nn.Dropout(p=0.3), # dropot的概率可以再调调
            nn.Linear(D_in,H),
            nn.ReLU(),
            nn.Linear(H, D_out)  # 全连接
        )
